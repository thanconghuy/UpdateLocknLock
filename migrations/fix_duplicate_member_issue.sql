...
-- =====================================================
-- Vấn đề: get_available_users_for_project chỉ filter status='active'
-- → User với status='removed' vẫn hiển thị → Add lại → Duplicate key error
--
-- Giải pháp: Filter TẤT CẢ users đã có record trong project_members
-- =====================================================
...
BEGIN
  ...
  -- Return users chưa có BẤT KỲ record nào trong project_members
  -- (Bất kể status là active, removed, hay suspended)
  RETURN QUERY
  SELECT
    up.id AS user_id,
    up.email,
  ...
  WHERE up.is_active = TRUE
    AND up.id NOT IN (
      ...
      WHERE pm.project_id = p_project_id
      -- ⚠️ FIX: Bỏ điều kiện status='active'
      -- → Filter ALL users đã có record (active/removed/suspended)
    )
  ORDER BY up.email;
END;
...
SELECT '✅ Fixed: get_available_users_for_project (prevent duplicate members)' as status;

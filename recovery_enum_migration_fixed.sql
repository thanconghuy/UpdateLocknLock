-- ====================================
-- RECOVERY ENUM MIGRATION - FIXED SYNTAX
...
BEGIN
    ...
    ELSIF has_role THEN
        ...
        -- Clean up previous attempts - FIX SYNTAX
        IF has_role_new THEN
            ALTER TABLE user_profiles DROP COLUMN role_new;
        END IF;
        IF has_role_v2 THEN
            ALTER TABLE user_profiles DROP COLUMN role_v2;
        END IF;
...
SELECT
    ...
    id,
    email,
    COALESCE(role_backup, 'unknown') as backup_role,
    COALESCE(role_v2::TEXT, 'missing') as new_role
FROM user_profiles
ORDER BY created_at;
...
SELECT
    ...
    COALESCE(role_backup, 'no backup') as original_backup,
    CASE
